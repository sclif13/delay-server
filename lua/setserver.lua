...
local uuid = session:getVariable("uuid") -- uuid звонка
...
if billsec then session:consoleLog("info",string.format("UUID=%s",uuid)); end

if ipt then
    if not cause then cause="FAIL" end
    if not cause then billsec=0 end
    if not uuid then uuid="NO UUID" end

    reqbody = "ip=" .. ipt .."&cause=" .. cause .."&billsec=" .. billsec .."&uuid=" .. uuid
    b, c, h = http.request {
          method = "POST",
